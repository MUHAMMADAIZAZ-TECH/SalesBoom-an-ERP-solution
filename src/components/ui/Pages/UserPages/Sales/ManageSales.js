...
const ManageSales = () => {
  ...
  const Remove = async (key) => {
    ...
    const HandleEdit = () =>{
      if(id!==""){
        History.push(`${id}/editsalesinvoice/`);
      }
      else{
        setMessage("Please Select an Item","error")
      }
  ...
  useEffect(() => {
    ...
    return (
     ...
     <div style={{ 
       ...
       <Typography variant="h6" component="h6" color="secondary">Manage Sales</Typography> 
         ...
         <Button color="primary" size="small"variant="contained" onClick={HandlePdf}><PictureAsPdfIcon /></Button>
          ...
          <Button color="primary" variant="contained" size="small" onClick={()=>deleteAllSalesInvoice()}>Delete All</Button>
         </div>
