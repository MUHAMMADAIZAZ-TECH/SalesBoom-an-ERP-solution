  ...
  const Columns = [
    ...
    return (
        <Container maxWidth="lg" className={classes.container}>
           <Typography variant="h6" component="h6" color="secondary">Manage Supliers</Typography> 
        <div className={classes.button}>
          <Button color="secondary" variant="contained" size="small" onClick={() => Remove(id)}>Delete</Button>
          <Button color="primary" variant="contained" size="small" onClick={()=>deleteAllSupliers()}>Delete All</Button>
          <Button  color="secondary" variant="contained" size="small"  onClick={()=>Update(id,Suplier)} >Save Changes</Button>
         </div>
          <div style={{ height: 400, width: '100%' }}>
          <DataGrid rows={supliersList} columns={Columns} 
          onRowSelected={(e) => {
            setSuplier(e.data)
            setId(e.data.id)}}
            components={{
